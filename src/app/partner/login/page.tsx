...
import { partnerApi, partnerSession } from '@/lib/partner-api';
...
export default function PartnerLoginPage() {
  ...
  useEffect(() => {
    if (partnerSession.token) router.replace('/partner');
  }, [router]);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const r = await partnerApi.login({ email: email.trim(), password });
      partnerSession.set(r.token);
      ...
      <div className="w-full max-w-md">
        <div className="rounded-3xl bg-white border border-ink-100 shadow-soft p-8">
          <div className="flex items-center gap-3 mb-6">
            <span className="grid place-items-center w-11 h-11 rounded-2xl bg-gradient-to-br from-brand-500 to-brand-700 text-white shadow-glow font-bold">
              ST
            </span>
            <div>
              <h1 className="font-extrabold tracking-tight text-lg leading-tight">
                Sinai<span className="text-brand-500">Taxi</span> Partner
              </h1>
              <p className="text-xs text-ink-500 font-medium">Hourly Rental</p>
            </div>
          </div>

          <h2 className="text-2xl font-bold tracking-tight mb-1">Welcome back</h2>
          <p className="text-sm text-ink-500 mb-6">
            Sign in with your Sinai Taxi partner account — same credentials as the Partner mobile app.
          </p>
        ...
        <p className="mt-4 text-center text-xs text-ink-500">
          Customer? <Link href="/" className="font-semibold text-ink-700 hover:text-ink-900">Go to the storefront</Link>
        </p>
      </div>
    </div>
  );
